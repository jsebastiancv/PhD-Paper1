function [ ax, cb] = contourf_flux( time, Lstar, PSD )
...
c_max = 2;
c_min = -2; % number of orders of magnitude to plot
    
PSD(PSD < 10^c_min) = 10^c_min;
% PSD(PSD > 10^c_max) = 10^c_max;
...
% get position
P = ax.Position;
cb = colorbar; ylabel(cb,{'log_{10} Flux','(#/sr/s/cm^2/keV)'});
% draw colorbar;
drawnow;
