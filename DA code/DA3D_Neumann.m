    ...
    if ~model_simulation_only
        ...
        for ispc=1:nspc
            ...
            try
                ...
                while cdate < ed
                    ...
                    file = [sdir,spc,'_',instr,'_',dates,'_psd_',hptxt,mfmtxt,'_ver4.mat'];
                    if load_errors_from_file
                        try
                            load(file);
                    ...
                    load(file);
                    file = [sdir,spc,'_',instr,'_',dates,'_lstar_',hptxt,mfmtxt,'_ver4.mat'];
                    load(file);
                    file = [sdir,spc,'_',instr,'_',dates,'_invmu_and_invk_',hptxt,mfmtxt,'_ver4.mat'];
                    load(file);


                    if strcmp(instr,'MAGEDandEPEAD')
                        file = [sdir,spc,'_',instr,'_',dates,'_alpha_and_energy_',...
                            hptxt,mfmtxt,'_ver4.mat']; 
                        load(file)
        ...
        %                errPSD = errPSD(:,ok_ech,:);
                    elseif strcmp(instr,'mageis')
                        file = [sdir,spc,'_',instr,'_',dates,'_alpha_and_energy_',...
                            hptxt,mfmtxt,'_ver4.mat']; load(file);
...
%         Pdyn_sim = Pdyn(it_tot,2);  % uncomment for EMIC scattering      
        
        save_spectrum
% 
%         Mp_it = Mp(it_tot,:); % uncomment for magnetopause losses
%         Mp_it(1) = 0;% 0.0416667;
% 
%         extra = Mp_it;
%         extra(1) = 0.0416667;
...
%         min_mp = min(Mp_it(2:end));
        ...
        %% 1. Forecast performed with one-step VERB
...
%         if Pdyn_sim > 3  % uncomment for EMIC scattering and magnetopause
%         losses
%              if min_mp < 6.6
...
%         else
             if min_mp < 6.6
                 mat_file = ['Model Matrices/Debug_ZeroVal/Kp=',num2str(Kp_sim),'.mat'];
             else
                mat_file = ['Model Matrices/Debug_ZeroDer/Kp=',num2str(Kp_sim),'.mat'];
             end
%         end
            load(mat_file);
